...
DevelopVersionString=`grep -E 'version.develop.*=' A4xLiveSDK_Extension.podspec`
DevelopVersionNumber=`tr -cd 0-9 <<<"$DevelopVersionString"`
NewDevelopVersionNumber=$DevelopVersionNumber

TestVersionString=`grep -E 'version.test.*=' A4xLiveSDK_Extension.podspec`
TestVersionNumber=`tr -cd 0-9 <<<"$TestVersionString"`
NewTestVersionNumber=$TestVersionNumber

GrayVersionString=`grep -E 'version.gray.*=' A4xLiveSDK_Extension.podspec`
...
DevelopVersionLineNumber=`grep -nE 'version.develop.*=' A4xLiveSDK_Extension.podspec | cut -d : -f1`
sed -i "" "${DevelopVersionLineNumber}s/${DevelopVersionNumber}/${NewDevelopVersionNumber}/g" A4xLiveSDK_Extension.podspec
...
TestVersionLineNumber=`grep -nE 'version.test.*=' A4xLiveSDK_Extension.podspec | cut -d : -f1`
sed -i "" "${TestVersionLineNumber}s/${TestVersionNumber}/${NewTestVersionNumber}/g" A4xLiveSDK_Extension.podspec
echo "current test version is ${TestVersionNumber}, new version is ${NewTestVersionNumber}, line number is ${TestVersionLineNumber}"

GrayVersionLineNumber=`grep -nE 'version.gray.*=' A4xLiveSDK_Extension.podspec | cut -d : -f1`
sed -i "" "${GrayVersionLineNumber}s/${GrayVersionNumber}/${NewGrayVersionNumber}/g" A4xLiveSDK_Extension.podspec
echo "current gray version is ${GrayVersionNumber}, new version is ${NewGrayVersionNumber}, line number is ${GrayVersionLineNumber}"

VersionString=`grep -E 's.version.*=' A4xLiveSDK_Extension.podspec`
...
VersionLineNumber=`grep -nE 's.version.*=' A4xLiveSDK_Extension.podspec | cut -d : -f1`
sed -i "" "${VersionLineNumber}s/${VersionNumber}/${NewVersionNumber}/g" A4xLiveSDK_Extension.podspec
...
pod repo push ${Repo} A4xLiveSDK_Extension.podspec --verbose --allow-warnings --use-libraries --use-modular-headers
